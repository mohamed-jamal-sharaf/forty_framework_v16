...
def _get_readable_timestamp():
    """
    Get a readable timestamp for backup files.
    Format: 31-Jan-2025_10h-22m-23s
    """
    now = datetime.now()
    return now.strftime("%d-%b-%Y_%Hh-%Mm-%Ss")


def _format_backup_timestamp_for_display(filename, original_filename):
    """
    Convert backup filename to readable display format.
    Input: hooks.py.bak.31-Jan-2025_10h-22m-23s
    Output: 31 Jan 2025 at 10:22:23
    """
    prefix = f"{original_filename}.bak."
    if not filename.startswith(prefix):
        return filename
    
    timestamp_part = filename.replace(prefix, "")
    
    try:
        # Parse: 31-Jan-2025_10h-22m-23s
        if "_" in timestamp_part and "h-" in timestamp_part:
            date_part, time_part = timestamp_part.split("_")
            # date_part: 31-Jan-2025
            # time_part: 10h-22m-23s
            
            day, month, year = date_part.split("-")
            time_clean = time_part.replace("h-", ":").replace("m-", ":").replace("s", "")
            
            return f"{day} {month} {year} at {time_clean}"
        else:
            # Old format fallback: 20250131-102223
...
def write_file_content_with_backup(app_name, file_path, content, password):
    ...
    # Create readable timestamp: 31-Jan-2025_10h-22m-23s
    ts = _get_readable_timestamp()
    ...
    # Format display timestamp: 31 Jan 2025 at 10:22:23
    display_ts = _format_backup_timestamp_for_display(backup_filename, filename)
    
    return {
        "success": True,
        "message": f"Saved successfully! Backup created: {display_ts}",
        ...
        for fn in sorted(os.listdir(backup_folder), reverse=True):
            if fn.startswith(prefix):
                ...
                try:
                    ...
                    # Get readable timestamp for display
                    display_timestamp = _format_backup_timestamp_for_display(fn, filename)
                    raw_timestamp = fn.replace(prefix, "")
...
def restore_backup(app_name, file_path, backup_filename, password):
    ...
    # Create pre-restore backup with readable timestamp
    ts = _get_readable_timestamp()
    ...
    # Get display timestamp from backup filename
    display_ts = _format_backup_timestamp_for_display(backup_filename, filename)
    
    return {
        "success": True,
        "message": f"Restored from backup: {display_ts}"
    }
