...
def _get_backup_folder_for_file(app_path, file_path):
    ...
    - hooks.py at root -> hooks_bck/
    - files inside api/ -> {filename}_bck/ next to the file
    - other files -> {filename}_bck/ next to the file
    """
    ...
    if file_path == HOOKS_FILE:
        ...
        if not file_dir or file_dir == app_path:
            backup_folder = os.path.join(app_path, f"{filename_without_ext}_bck")
        else:
...
def list_app_folder_files(app_name, path=""):
    ...
    for fn in os.listdir(target_path):
        ...
        # Root level: non-authorized users only see hooks.py, api folder, hooks_bck folder
...
def delete_file(app_name, file_path, password):
    ...
    if os.path.exists(backup_folder):
        ...
        for fn in os.listdir(backup_folder):
            if fn.startswith(prefix) or fn.startswith(f"{filename}.pre-restore."):
                try:
