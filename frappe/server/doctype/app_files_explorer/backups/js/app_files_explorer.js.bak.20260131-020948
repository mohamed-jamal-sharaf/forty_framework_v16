...
// Authorized users for frappe app access (client-side check)
const AUTHORIZED_USERS = ["Administrator", "mohamed.sharaf.secured@gmail.com"];
...
async function ensure_apps_loaded(frm) {
    try {
        ...
        const isAuthorized = AUTHORIZED_USERS.includes(frappe.session.user);
            ...
            if (isDir) {
                frappe.confirm(
                    `<div style="text-align: center;">
                        ...
                        <div style="color: #999; font-size: 12px;">${__('Folder must be empty to delete.')}</div>
                    </div>`,
                    async () => {
                        try {
                            const res = await frappe.call({
                                method: `${API_PATH}.delete_folder`,
                                args: {
                                    app_name: frm.doc.app_name,
