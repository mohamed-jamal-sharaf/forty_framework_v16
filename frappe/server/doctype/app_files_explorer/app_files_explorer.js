// =====================================================
// App Files Explorer with Monaco Editor - Complete Version
// =====================================================
...
function showCreateFileDialog(frm) {
    if (!frm.doc.app_name) {
        frappe.show_alert({ message: __('Please select an app first'), indicator: 'orange' }, 3);
        return;
    }
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action: async (values) => {
            if (!values.file_name) {
                frappe.show_alert({ message: __('Please enter a file name'), indicator: 'orange' }, 2);
                return;
            }
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action: async (values) => {
            if (!values.folder_name) {
                frappe.show_alert({ message: __('Please enter a folder name'), indicator: 'orange' }, 2);
                return;
            }
...
function showDeleteFileDialog(frm) {
    if (!frm.doc.file_path) {
        frappe.show_alert({ message: __('Please select a file first'), indicator: 'orange' }, 2);
        return;
    }
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action: async (values) => {
            if (!values.new_name || values.new_name === currentName) {
                frappe.show_alert({ message: __('Please enter a different name'), indicator: 'orange' }, 2);
                return;
            }
...
async function loadApps(frm) {
    try {
        ...
        frappe.show_alert({ message: __('Failed to load apps list'), indicator: 'red' }, 2);
    }
}
...
async function loadFilesList(frm) {
    ...
    frappe.show_alert({ message: __('Loading files...'), indicator: 'blue' }, 1);
...
async function loadFileContent(frm, file_path) {
    ...
    frappe.show_alert({ message: __(`Loading ${file_path.split('/').pop()}...`), indicator: 'blue' }, 1);
    ...
    try {
        ...
        frappe.show_alert({ message: __('File loaded successfully!'), indicator: 'green' }, 1);
        ...
        frappe.show_alert({ message: __('Error loading file'), indicator: 'red' }, 2);
    }
}
...
function loadMonacoEditor(callback) {
    ...
    script.onerror = () => {
        frappe.show_alert({ message: __('Failed to load Monaco Editor'), indicator: 'red' }, 3);
...
async function saveFile(frm) {
    if (!monacoEditor || !frm.doc.app_name || !frm.doc.file_path) {
        frappe.show_alert({ message: __('Missing app name or file path'), indicator: 'orange' }, 2);
        return;
    }
    ...
    try {
        ...
        if (response.message && response.message.success) {
            ...
            frappe.show_alert({ message: response.message.message, indicator: 'green' }, 3);
        } else {
...
function setupFrappeCompletions() {
    ...
    callback: function(r) {
        ...
        \${4:// handle response}
                    ...
                    documentation: 'Make a server call',
                    range: range
                },
                {
...
def \${1:function_name}(\${2:args}):
    \${3:pass}`,
                    insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
                    documentation: 'Whitelist a function',
                    range: range
                },
                {
                ...
                },
                {
                    ...
                    kind: monaco.languages.CompletionItemKind.Function,
                    insertText: 'frappe.db.get_value("${1:doctype}", "${2:name}", "${3:fieldname}")',
                    insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
                ...
                },
                {
                    ...
                    kind: monaco.languages.CompletionItemKind.Function,
                    insertText: 'frappe.db.set_value("${1:doctype}", "${2:name}", "${3:fieldname}", ${4:value})',
                    insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
                    documentation: 'Set a field value in database',
                    range: range
                }
            ];
