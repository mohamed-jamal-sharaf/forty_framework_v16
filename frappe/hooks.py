...
scheduler_events = {
    "cron": {
        "* * * * *": [
            "frappe.web_form_utils.update_webform_publish"
        ]
    }
}
...
doc_events = {
	"*": {
        ...
        # Custom Forty
        # "onload":"frappe.api.custom_api.standard_field.fill_fields", 
	},
	"Event": {
	...
	"DocType": {
        ...
        # Custom Forty
        # "after_insert":"frappe.api.custom_api.standard_field.auto_generate_fields",
	},
	"Page": {
...
# In frappe/frappe/hooks.py

permission_query_conditions = {
    "DocType": "frappe.hooks.doctype_query_conditions",
}

has_permission = {
    "DocType": "frappe.hooks.doctype_has_permission",
}

def doctype_query_conditions(user):
    """Filter DocTypes by module"""
    import frappe
    ...
    hidden_modules = ['Core', 'Desk', 'Geo', 'Custom', 'Website', 'Automation', 'Email', 'Printing','Workflow','Integrations', 'Contacts','Job Application Tracking System','SMS MISR' ]
    
    # Return only string, not tuple
    module_list = ', '.join([frappe.db.escape(module) for module in hidden_modules])
    return f"(`tabDocType`.`module` NOT IN ({module_list}))"


def doctype_has_permission(doc, ptype, user):
    """Check permission for individual DocType"""
    import frappe
    ...
    return doc.module not in hidden_modules
