...
def before_save(doc, method=None):
    doc.published = 1 if _is_active_now(
        start_date=doc.start_date,
        end_date=doc.end_date,
        start_time=(doc.start_time or "00:00:00"),
        end_time=(doc.end_time or "23:59:59"),
    ) else 0

def _is_active_now(start_date=None, end_date=None, start_time="00:00:00", end_time="23:59:59"):
    now = now_datetime().replace(microsecond=0)
    sd = getdate(start_date) if start_date else getdate(now)
    ed = getdate(end_date) if end_date else getdate(now)
    start_dt = get_datetime(f"{sd} {start_time}")
    end_dt   = get_datetime(f"{ed} {end_time}")
    # normal vs overnight window
    return (start_dt <= end_dt and start_dt <= now <= end_dt) or (start_dt > end_dt and (now >= start_dt or now <= end_dt))
